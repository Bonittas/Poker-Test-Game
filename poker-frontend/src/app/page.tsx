...
const Home: NextPage = () => {
  ...
  const [gameState, setGameState] = useState<GameState>({
    ...
    playerStacks: PLAYERS.map(() => stackSize),
  ...
  const handleAction = (action: string, amount?: number) => {
    ...
    let newPot = pot;
    const newLastActions = [...gameState.lastActions];
    ...
    switch (action) {
      ...
      case "bet":
        if (amount !== undefined) {
          ...
          newPot += amount;
          newActionSequence += newActionSequence ? ` b${amount}` : `b${amount}`;
    ...
    <main className="container mx-auto p-6">
      ...
      <Card className="mb-6 bg-card border-border shadow-sm">
        <div className="p-2">
          ...
          <Card className="bg-card mb-6 border-border shadow-sm">
            <div className="p-6">
              <PokerForm
